...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("CareerCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CareerCoinTEST2\r");
            Send(hSocket, "WHO #CareerCoinTEST2\r");
        } else {
            // randomly join #CareerCoin00-#CareerCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #CareerCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CareerCoin%02d\r", channel_number).c_str());
        }
